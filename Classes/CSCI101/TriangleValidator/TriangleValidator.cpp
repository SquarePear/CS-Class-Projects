...
int main() {
  ...
  std::cout << ((isValidTriangle) ? "Triangle is valid!"
                                  : "Triangle is invalid!")
            << std::endl;

  // If triangle is not valid, exit. An invalid triangle cannot be a right
  // triangle.
  if (!isValidTriangle)
    return 0;
  ...
  std::cout << ((isRightAngle) ? "Triangle is right angle!"
