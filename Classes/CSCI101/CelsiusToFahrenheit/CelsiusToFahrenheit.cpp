...
int main() {
  ...
  do {
    std::cout << "Enter a value for the lower Celsius temperature (whole "
                 "numbers only): ";
    std::cin >> lowerCelsius;

    std::cout << "Enter a value for the upper Celsius temperature (whole "
                 "numbers only): ";
  ...
  for (int i = lowerCelsius; i <= upperCelsius; i++) {
    ...
    std::cout << " | ";
    std::cout << std::setw(8) << std::right << celsiusToFahrenheit(i) << "°F"
              << std::endl;
  }
...
double celsiusToFahrenheit(int celsius) { return 1.8 * celsius + 32; }