...
int main() {
  ...
  std::cout << "The average low recorded temperature was " << averageLowTemp << " degrees." << std::endl;
  std::cout << "The average high recorded temperature was " << averageHighTemp << " degrees." << std::endl;
  std::cout << "The lowest recorded temperature was in " << MONTHS[lowTempIndex] << " at " << rainfall[lowTempIndex][0] << " degrees." << std::endl;
  std::cout << "The highest recorded temperature was in " << MONTHS[highTempIndex] << " at " << rainfall[highTempIndex][1] << " degrees." << std::endl;
...
void getData(double rainfall[TOTAL_MONTHS][2]) {
  for (int i = 0; i < TOTAL_MONTHS; i++) {
    ...
    while (!(std::cin >> highTemp) || highTemp < lowTemp) {
      std::cout << "Invalid input! Value must be greater that the lowest temperature. Please enter again: ";
      std::cin.clear();
      std::cin.ignore(1000, '\n');
    }
...
int indexLowTemp(double rainfall[TOTAL_MONTHS][2]) {
  ...
  for (int i = 1; i < TOTAL_MONTHS; i++) {
    if (rainfall[indexLow][0] > rainfall[i][0]) indexLow = i;
  }
...
int indexHighTemp(double rainfall[TOTAL_MONTHS][2]) {
  ...
  for (int i = 1; i < TOTAL_MONTHS; i++) {
    if (rainfall[indexHigh][1] < rainfall[i][1]) indexHigh = i;
  }
