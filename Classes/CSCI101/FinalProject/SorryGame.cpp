...
void movePlayer(Player players[MAX_PLAYERS], int playerCount, int currentPlayer, int spaces);
RollInfo rollDie(int rollCount[11]);
void runDiceAction(RollInfo roll, Player players[MAX_PLAYERS], int playerCount, int currentPlayer);
...
int main() {
  ...
  do {
    ...
    std::cout << "How many players are there? (" << MIN_PLAYERS << "-" << MAX_PLAYERS << "): ";
    while (!(std::cin >> playerCount) || playerCount < MIN_PLAYERS || playerCount > MAX_PLAYERS) {
    ...
    while (gameState == 0) {
      ...
      for (int i = 0; i < playerCount; i++) {
        ...
        std::cout << "You rolled a " << roll.total << " (" << roll.diceValues[0] << "+" << roll.diceValues[1] << ")" << std::endl;
        ...
        if (players[i].position == FINAL_SPACE) {
          ...
          if (players[i].rolls[1].doubles && players[i].rolls[0].doubles) {
            std::cout << players[i].name << " rolled doubles twice in a row and had to start over!" << std::endl;

            players[i].rolls[0].doubles = false;

            players[i].position = 0;
          } else if (players[i].rolls[0].doubles) {
            std::cout << players[i].name << " rolled doubles and they get an extra turn!" << std::endl;
            i--;
          }
        }
    ...
    std::cout << "Congratulations " << players[gameState - 1].name << "! You win!" << std::endl << std::endl;

    for (int i = 2; i <= 12; i++) std::cout << i << ": " << rollCount[i - 2] << std::endl;

    // Restart logic
    char restartChar;
    std::cout << "Do you want to play again? (y/n): ";

    while (!(std::cin >> restartChar) || (restartChar != 'n' && restartChar != 'y')) {
...
void movePlayer(Player players[MAX_PLAYERS], int playerCount, int currentPlayer, int spaces) {
  ...
  if (players[currentPlayer].position + spaces <= 0) {
    std::cout << players[currentPlayer].name << " tried to move backwards before the starting space and fell off the board!" << std::endl;
    players[currentPlayer].position = 0;
    return;
  } else if (players[currentPlayer].position + spaces > FINAL_SPACE) {
    std::cout << players[currentPlayer].name << " tried to move past the final space, but they failed!" << std::endl;
    return;
  }

  players[currentPlayer].position += spaces;

  std::cout << players[currentPlayer].name << " moved " << spaces << " spaces!" << std::endl;
  ...
  for (int i = 0; i < playerCount; i++) {
    ...
    if (players[i].position == players[currentPlayer].position) {
      ...
      std::cout << players[i].name << " was kicked off the board by " << players[currentPlayer].name << "!" << std::endl;
    }
  }
}
...
void displayBoard(Player players[MAX_PLAYERS], int playerCount) {
  ...
  for (int i = 0; i < 60; i++) {
    ...
    for (int j = 0; j < playerCount; j++) {
      if (players[j].position - 1 == i) player = j;
    }
  ...
  for (int i = 0; i < playerCount; i++) {
    std::cout << "P" << i + 1 << ") " << players[i].name << ": " << players[i].position << std::endl;
  }
...
void runDiceAction(RollInfo roll, Player players[MAX_PLAYERS], int playerCount, int currentPlayer) {
  ...
  if (players[currentPlayer].position == 0) {
    if (roll.doubles) {
      ...
      std::cout << players[currentPlayer].name << " rolled doubles and landed on the board!" << std::endl;
    }
  ...
  switch (roll.total) {
    ...
    case 11:
      ...
      break;
    case 12:
      std::cout << players[currentPlayer].name << " was sent off the board!" << std::endl;
      players[currentPlayer].position = 0;
      break;
    default:
      movePlayer(players, playerCount, currentPlayer, roll.total);
...
int getFirstPlayer(Player players[MAX_PLAYERS], int playerCount) {
  ...
  for (int i = 0; i < playerCount; i++) {
    if (players[i].position > ((playerIndex == -1) ? 0 : players[playerIndex].position)) playerIndex = i;
  }
...
int getLastPlayer(Player players[MAX_PLAYERS], int playerCount) {
  ...
  for (int i = 0; i < playerCount; i++) {
    if ((playerIndex == -1) ? (players[i].position > 0) : (players[i].position > 0 && players[i].position < players[playerIndex].position)) playerIndex = i;
  }
...
void swapPlayers(Player players[MAX_PLAYERS], int playerA, int playerB) {
  ...
  if (playerB == -1) {
    std::cout << players[playerA].name << " tried to swap places with someone, but there was nobody on the board!" << std::endl;
    return;
  }

  // Stop if player A and B are the same
  if (playerA == playerB) {
    std::cout << players[playerA].name << " tried to swap places with themselves, but they failed!" << std::endl;
    return;
  }
  ...
  std::cout << players[playerA].name << " swapped positions with " << players[playerB].name << "!" << std::endl;
}